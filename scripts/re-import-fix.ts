...
async function main() {
  ...
  try {
    ...
    const {
      LocationsService,
    } = require("../apps/api/src/locations/locations.service");
    ...
    const locPath = path.join(
      ...
      "docs",
      "samples",
      "sample-locations-import.xlsx",
    );
    const locBuffer = fs.readFileSync(locPath);
    const locResults = await locService.bulkImport(
      locBuffer,
      "sample-locations-import.xlsx",
    );
    ...
    const invResults = await invService.bulkImport(
      invBuffer,
      "sample-inventory-import.xlsx",
    );
