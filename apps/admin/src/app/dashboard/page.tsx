...
import { Package, MapPin, FileCheck, QrCode, TrendingUp, BarChart3 } from 'lucide-react';
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-slate-400 text-sm mt-1">Asset Verification System Overview</p>
            </div>
                            ...
                            {audits.reports.slice(0, 5).map((report: any) => (
                                <div key={report.id} className="flex items-center justify-between py-2 border-b border-slate-800/30 last:border-0">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500">{report.auditor?.name} — {new Date(report.createdAt).toLocaleDateString()}</p>
                                    </div>
                                    <span className={`text-xs px-2 py-1 rounded-full font-medium
                                        ${report.status === 'APPROVED' ? 'bg-emerald-500/15 text-emerald-400'
                                            : report.status === 'SUBMITTED' ? 'bg-amber-500/15 text-amber-400'
                                                : report.status === 'REJECTED' ? 'bg-red-500/15 text-red-400'
                    ...
                    {locations?.length ? (
                        <div className="space-y-3">
                            {locations.slice(0, 6).map((loc: any) => (
                                <div key={loc.id} className="flex items-center justify-between py-2 border-b border-slate-800/30 last:border-0">
                                    <div className="flex items-center gap-2">
                                        <div className="w-2 h-2 rounded-full bg-emerald-500" />
                                        <p className="text-sm text-white">{loc.name}</p>
                                        <span className="text-xs text-slate-600">{loc.code}</span>
                                    </div>
                                    <span className="text-xs text-slate-500">{loc._count?.inventoryItems || 0} items</span>
                                </div>
                            ))}
                        </div>
                    ) : (
