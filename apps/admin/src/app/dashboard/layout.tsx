...
import {
    LayoutDashboard, MapPin, Package, FileCheck, QrCode,
    LogOut, ChevronLeft, ChevronRight, ShieldCheck, User, Menu, Upload
} from 'lucide-react';
...
const navItems = [
    ...
    { href: '/dashboard/audits', label: 'Audit Reports', icon: FileCheck },
    { href: '/dashboard/qr-tags', label: 'QR Tags', icon: QrCode },
    { href: '/dashboard/imports', label: 'Data Import', icon: Upload }, // Use Upload icon or similar
];
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-slate-950 flex">
            ...
            `}>
                {/* Logo */}
                <div className={`flex items-center gap-3 px-4 h-16 border-b border-slate-800/50 ${collapsed ? 'justify-center' : ''}`}>
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <header className="h-16 border-b border-slate-800/50 bg-slate-950/80 backdrop-blur-sm flex items-center px-4 lg:px-6 sticky top-0 z-30">
                    <button onClick={() => setMobileOpen(true)} className="lg:hidden mr-3 text-slate-400 hover:text-white">
