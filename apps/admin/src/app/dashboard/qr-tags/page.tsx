...
export default function QrTagsPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => api.getQrTags({ status: status || undefined, page: String(page), limit: '25' } as any),
    });
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">Manage QR code tags for asset verification</p>
                </div>
                <button
            ...
            {showGenerate && (
                <div className="bg-slate-900/80 border border-blue-500/20 rounded-2xl p-5">
                    <h3 className="text-sm font-semibold text-white mb-4">Generate QR Tag Batch</h3>
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                    {data.tags.map((tag: any) => (
                        <div key={tag.id} className="bg-slate-900/80 border border-slate-800/50 rounded-xl p-4 hover:border-slate-700/50 transition-all">
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                    <QrCode className="w-4 h-4 text-slate-500" />
                                    <span className="font-mono text-sm text-white font-medium">{tag.code}</span>
                                </div>
                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColors[tag.status] || statusColors.UNASSIGNED}`}>
                                    {tag.status}
                                </span>
                            </div>
                            {tag.linkedItem && (
