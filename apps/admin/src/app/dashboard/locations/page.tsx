...
function LocationNode({ location, depth = 0 }: { location: any; depth?: number }) {
    ...
    return (
        <div>
            ...
            >
                ...
                <div className={`w-2 h-2 rounded-full ${depth === 0 ? 'bg-blue-500' : depth === 1 ? 'bg-emerald-500' : depth === 2 ? 'bg-amber-500' : 'bg-purple-500'}`} />
                ...
                <span className="text-xs text-slate-600 px-2 py-0.5 rounded bg-slate-800/50">{location.levelLabel}</span>
            </div>
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-400 text-sm mt-1">Manage your location hierarchy</p>
                </div>
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800/50 border border-slate-700/50">
                    <FolderTree className="w-4 h-4 text-slate-500" />
                    <span className="text-sm text-slate-300">{flat?.length || 0} locations</span>
                </div>
            </div>
