...
import { Upload, Trash2, AlertTriangle, CheckCircle, XCircle } from 'lucide-react';

export default function DataImportPage() {
    const { } = useAuth(); // Auth context not needed for token anymore as api client handles it
    ...
    const handleWipeData = async () => {
        if (!confirm('Are you ABSOLUTELY SURE? This will delete ALL Locations and Inventory items. This action cannot be undone.')) {
            return;
        }
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-400">Manage bulk data imports for locations and inventory.</p>
                </div>
                <div className="flex gap-2">
                    ...
                    >
                        {wiping ? <div className="w-4 h-4 border-2 border-red-400 border-t-transparent rounded-full animate-spin" /> : <Trash2 className="w-4 h-4" />}
                ...
                <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400">
                    ...
                    {result.errors && result.errors.length > 0 && (
                        <div className="mt-3 p-3 bg-slate-900/50 rounded-lg text-sm text-slate-300 max-h-40 overflow-y-auto">
                            <p className="font-medium text-amber-400 mb-1">{result.errorCount} Errors Found:</p>
                            <ul className="space-y-1">
                                {result.errors.map((e: any, i: number) => (
                                    <li key={i} className="flex gap-2">
                                        <span className="text-slate-500">Row {JSON.stringify(e.row)}:</span>
            ...
            <div className="flex border-b border-slate-800">
                <button
                    onClick={() => { setActiveTab('locations'); setResult(null); setError(null); }}
                ...
                <button
                    onClick={() => { setActiveTab('inventory'); setResult(null); setError(null); }}
                ...
                <div className="flex items-start gap-3">
                    ...
                    <div className="text-sm text-slate-300 space-y-1">
                        ...
                        <ul className="list-disc pl-4 space-y-1 text-slate-400">
                            <li>Data wipe is <strong>irreversible</strong>.</li>
                            <li>For Locations: Ensure <strong>Code</strong> and <strong>Name</strong> columns exist. Parent codes must precede children or exist already.</li>
                            <li>For Inventory: Ensure <strong>Asset Code</strong> and <strong>Location Code</strong> exist. Locations must exist before importing inventory.</li>
                        </ul>
                    </div>
                </div>
            </div>
