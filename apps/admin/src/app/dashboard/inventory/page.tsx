        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-slate-400 text-sm mt-1">
                    Manage assets and equipment — {data?.pagination?.total || 0} total items
                </p>
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-800/50 bg-slate-800/20">
                                ...
                                <th className="text-left px-4 py-3 text-slate-400 font-medium">QR Tag</th>
                            </tr>
                        </thead>
                        <tbody>
                            {isLoading ? (
                                ...
                                data.items.map((item: any) => (
                                    <tr key={item.id} className="border-b border-slate-800/30 hover:bg-slate-800/20 transition-colors">
                                        <td className="px-4 py-3">
                                            <span className="font-mono text-xs bg-slate-800/50 px-2 py-0.5 rounded text-blue-300">{item.code}</span>
                                        </td>
                                        <td className="px-4 py-3 text-white font-medium">{item.name}</td>
                                        <td className="px-4 py-3 text-slate-400">{item.location?.name || '—'}</td>
                                        <td className="px-4 py-3 text-slate-400">{item.department?.name || '—'}</td>
                                        <td className="px-4 py-3 text-slate-400">{item.category?.name || '—'}</td>
                                        <td className="px-4 py-3">
                                            {item.qrTag ? (
                                                <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400">{item.qrTag.code}</span>
                                            ) : (
