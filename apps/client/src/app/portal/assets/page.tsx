...
export default function AssetsPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => api.getInventory({ search: search || undefined, page: String(page), limit: '20' } as any),
    });
    ...
    return (
        <div className="space-y-6">
            <div>
                ...
                <p className="text-slate-400 text-sm mt-1">{data?.pagination?.total || 0} registered assets</p>
            </div>
            ...
            <div className="bg-slate-900/80 border border-slate-800/50 rounded-2xl overflow-hidden">
                                ...
                                data.items.map((item: any) => (
                                    <tr key={item.id} className="border-b border-slate-800/30 hover:bg-slate-800/20 transition-colors">
                                        <td className="px-4 py-3">
                                            <span className="font-mono text-xs bg-slate-800/50 px-2 py-0.5 rounded text-emerald-300">{item.code}</span>
                                        </td>
                                        <td className="px-4 py-3 text-white font-medium">{item.name}</td>
                                        <td className="px-4 py-3 text-slate-400">{item.location?.name || '—'}</td>
                                        <td className="px-4 py-3 text-slate-400">{item.category?.name || '—'}</td>
                                        <td className="px-4 py-3">
                                            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${item.qrTag ? 'bg-emerald-500/15 text-emerald-400' : 'bg-slate-500/15 text-slate-400'
                                                }`}>{item.qrTag ? 'Tagged' : 'Untagged'}</span>
                                        </td>
                                    </tr>
                                ))
                            ) : (
                                <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-500">No assets found</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
                ...
                {data?.pagination && data.pagination.totalPages > 1 && (
                    <div className="flex items-center justify-between px-4 py-3 border-t border-slate-800/50">
                        <span className="text-xs text-slate-500">Page {data.pagination.page} of {data.pagination.totalPages}</span>
