...
export class LocationsController {
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: "Filter by depth level (0-4)",
  })
  async findAll(
  ...
  @Get(":id/subtree")
  @ApiOperation({
    summary: "Get all location IDs in the subtree (cascading access)",
  })
  @ApiResponse({
    status: 200,
    description: "Array of location IDs in the subtree",
  })
    ...
    FileInterceptor("file", {
      ...
      fileFilter: (_req, file, cb) => {
        ...
        if (
          ...
          cb(
            new Error("Only Excel (.xlsx/.xls) and CSV files are supported"),
            false,
          );
        }
      },
  ...
  @ApiOperation({
    ...
    description:
      "Upload a file with columns: L1 Code, L1 Name, L2 Code, L2 Name, ... L5 Code, L5 Name. " +
