...
export class LocationsService {
    ...
    async findOne(id: string) {
        const location = await this.prisma.location.findUnique({
            ...
            include: {
                ...
                children: { select: { id: true, code: true, name: true, depth: true, levelLabel: true } },
    ...
    async create(dto: CreateLocationDto) {
        ...
        const existing = await this.prisma.location.findFirst({ where: { code: dto.code } });
        if (existing) {
     ...
     */
    async bulkImport(fileBuffer: Buffer, originalName: string): Promise<{
        created: number;
        updated: number;
        skipped: number;
        errors: string[];
        ...
        for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
            ...
            try {
                ...
                for (let level = 0; level < MAX_DEPTH; level++) {
                    const code = this.cleanCell(row[`L${level + 1} Code`] || row[`l${level + 1}_code`] || row[`L${level + 1}Code`]);
                    const name = this.cleanCell(row[`L${level + 1} Name`] || row[`l${level + 1}_name`] || row[`L${level + 1}Name`]);

                    // If both code and name are empty, this level and beyond are empty (ragged tree)
                    if (!code && !name) break;

                    if (!code || !name) {
                        errors.push(`Row ${rowNum}: Level ${level + 1} has code but no name (or vice versa)`);
                        break;
                    }
                    ...
                    const existing = await this.prisma.location.findFirst({ where: { code } });
    ...
    private parseFile(buffer: Buffer, filename: string): Record<string, string>[] {
        ...
        if (rows.length > 0) {
            ...
            const hasL1 = headers.some(h =>
                /l1[\s_]?code/i.test(h) || h === 'L1 Code' || h === 'L1Code'
            );
            if (!hasL1) {
                throw new BadRequestException(
