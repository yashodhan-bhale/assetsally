...
describe("QrTagsService", () => {
  ...
  describe("findAll", () => {
    it("should return tags and pagination info", async () => {
      mockPrismaService.qRCodeTag.findMany.mockResolvedValue([
        { id: "1", code: "QR1" },
      ]);
  ...
  describe("initiateBatchGeneration", () => {
    ...
    it("should throw BadRequestException for invalid count", async () => {
      await expect(
        service.initiateBatchGeneration("http://localhost/q", 0, "user-1"),
  ...
  describe("assignToItem", () => {
    it("should assign tag to item in a transaction", async () => {
      ...
      mockPrismaService.qRCodeTag.update.mockResolvedValue({
        ...tag,
        status: QRTagStatus.ASSIGNED,
      });
      mockPrismaService.qRBindingRecord.create.mockResolvedValue({
        id: "binding-1",
        qrTagId: tag.id,
        itemId: item.id,
      });
    ...
    it("should throw BadRequestException if tag already assigned", async () => {
      mockPrismaService.qRCodeTag.findFirst.mockResolvedValue({
        status: QRTagStatus.ASSIGNED,
      });

      await expect(service.assignToItem("QR-001", "item-1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
