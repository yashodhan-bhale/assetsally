...
export class ImportsService {
    ...
    async wipeData() {
        try {
            ...
            return { message: 'All locations and inventory items have been wiped successfully.' };
        } catch (error) {
    ...
    async processImport(file: Express.Multer.File, type: 'locations' | 'inventory') {
    ...
    private async importLocations(data: any[]) {
        ...
        for (const row of sortedData) {
            try {
                ...
                const parentCode = String(row['Parent Code'] || row['parent_code'] || '').trim();
                ...
                if (parentCode) {
                    ...
                    if (parent) {
                        ...
                        errorCount++;
                        errors.push({ row, error: `Parent location with code ${parentCode} not found` });
                        continue;
                    }
                }
    ...
    private async importInventory(data: any[]) {
        ...
        for (const row of data) {
            try {
                const code = String(row['Asset ID'] || row['Asset Code'] || row['code'] || '').trim();
                const name = String(row['Description'] || row['name'] || '').trim();
                const locationCode = String(row['Location Code'] || row['location_code'] || '').trim();
                const departmentCode = String(row['Department Code'] || row['department_code'] || '').trim();
                const categoryCode = String(row['Category Code'] || row['category_code'] || '').trim();
                ...
                if (locationCode) {
                    const loc = await this.prisma.location.findFirst({ where: { code: locationCode } });
                    if (!loc) {
                        errorCount++;
                ...
                if (departmentCode) {
                    let dept = await this.prisma.department.findUnique({ where: { code: departmentCode } });
                    if (!dept) {
                        const deptName = String(row['Department'] || row['Department Name'] || departmentCode).trim();
                ...
                let categoryId = null;
                if (categoryCode) {
                    let cat = await this.prisma.assetCategory.findUnique({ where: { code: categoryCode } });
                    if (!cat) {
                        const catName = String(row['Asset Category'] || row['Category Name'] || categoryCode).trim();
                ...
                if (purchaseDateRaw) {
                    if (typeof purchaseDateRaw === 'number') {
                        purchaseDate = new Date(Math.round((purchaseDateRaw - 25569) * 86400 * 1000));
                    } else {
