...
import { VerificationStatus, ItemCondition, AuditReportStatus } from '@prisma/client';

@Injectable()
export class AuditsService {
    constructor(private prisma: PrismaService) { }

    async findAll(query?: { locationId?: string; auditorId?: string; status?: string; page?: number; limit?: number }) {
    ...
    async submitFinding(reportId: string, dto: SubmitFindingDto, auditorId: string) {
        const report = await this.prisma.auditReport.findUnique({ where: { id: reportId } });

        if (!report) throw new NotFoundException('Report not found');
        if (report.auditorId !== auditorId) throw new ForbiddenException('Not your report');
        if (report.status !== AuditReportStatus.DRAFT) throw new BadRequestException('Report already submitted');
    ...
    async submitReport(reportId: string, auditorId: string) {
        ...
        if (report.auditorId !== auditorId) throw new ForbiddenException('Not your report');
        if (report.status !== AuditReportStatus.DRAFT) throw new BadRequestException('Report already submitted');
        if (report._count.findings === 0) throw new BadRequestException('Cannot submit report with no findings');
    ...
    async reviewReport(reportId: string, dto: ReviewReportDto, reviewerId: string) {
        const report = await this.prisma.auditReport.findUnique({ where: { id: reportId } });

        if (!report) throw new NotFoundException('Report not found');
        if (report.status !== AuditReportStatus.SUBMITTED) throw new BadRequestException('Report not submitted for review');
