      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
            <button
              ...
              className={`px-4 py-1.5 text-sm font-medium rounded-lg transition-colors ${activeTab === "TAGS" ? "bg-white text-slate-900 shadow-sm border border-slate-200" : "text-slate-500 hover:text-slate-700"}`}
            >
              Inventory Tags
            </button>
            <button
              onClick={() => setActiveTab("JOBS")}
              className={`px-4 py-1.5 text-sm font-medium rounded-lg transition-colors ${activeTab === "JOBS" ? "bg-white text-slate-900 shadow-sm border border-slate-200" : "text-slate-500 hover:text-slate-700"}`}
            >
              Generation Jobs
            </button>
          </div>
        ...
        {activeTab === "TAGS" && (
          <div className="flex justify-end mb-4">
            <a
              href={`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001/api"}/qr-tags/export-bindings?token=${api.getToken()}`}
              target="_blank"
              rel="noreferrer"
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <span
                          className={`text-xs px-2.5 py-1 rounded-full font-medium ${jobStatusColors[job.status] || "bg-slate-100 text-slate-600"}`}
                        >
                          {job.status}
                        </span>
                      </div>
                            ...
                            {batches?.map(
                              (batch: {
                                ...
                                >
                                  ...
                                  <td className="p-4">
                                    <span
                                      className={`text-[11px] px-2.5 py-1 rounded-full font-bold uppercase tracking-wide border ${jobStatusColors[batch.status] || "bg-slate-100 text-slate-600 border-slate-200"}`}
                                    >
                                      {batch.status}
                                    </span>
                                  </td>
                                  <td className="p-4 text-right">
                                    <div className="flex justify-end gap-2">
                                      ...
                                      batch.status === "DOWNLOADED" ? (
                                        <>
                                          <a
                                            href={`${api.getBatchDownloadUrl(batch.id)}?token=${api.getToken()}`}
                                            target="_blank"
                                            rel="noreferrer"
                                            onClick={() => {
                                              setTimeout(
