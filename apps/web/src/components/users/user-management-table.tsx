...
interface UserManagementTableProps {
  title: string;
  description: string;
  appTypeFilter: "ADMIN" | "CLIENT" | "MOBILE";
  defaultRole: string;
}
...
export function UserManagementTable({
  ...
  const loadData = React.useCallback(async () => {
    ...
    try {
      ...
      // Filter by appType immediately
      const filtered = data.filter(
        (u: UserListItem) => u.appType === appTypeFilter,
      );
      setUsers(filtered);
    } catch (err) {
  ...
  const onSubmit = async (data: CreateUserFormValues) => {
    ...
    setError(null);
    try {
      await api.createUser({ ...data, appType: appTypeFilter });
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <select
                  {...register("appType")}
                  className="w-full px-3 py-2 rounded-lg border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-slate-50 cursor-not-allowed"
                  disabled
                >
